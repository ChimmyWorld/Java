...
public class Ex04 {
	public static void main(String[] args) {
		...
		// index가 없는 구조를 순환할 때 사용할 수 있다 (e.g. Set)
		for (int n : list) {
		...
		// 반복이 끝난다고 해서 iterator가 재 위치로 돌아가진 않음
		System.out.println("it 다음 ? " + it.hasNext()); // false
		
		// 다시 순환하려면 collection으로 부터 다시 받는다
		it = list.iterator();
		System.out.println("it 다음 ? "  + it.hasNext()); // true
	}
}
